...
export const startBattle = async (req, res) => {
  try {
    ...
    const { currentUser, opponent } = await getBattleOpponentService(
      currentUserId
    );
    ...
    if (!currentUser) {
      return res
        .status(400)
        .json({ message: "No tienes Pokémon para batallar" });
    }
    ...
    const myPokemon = {
      ...currentUser.pokemons[0],
      currentHp: currentUser.pokemons[0].hp,
    };
    const enemyPokemon = {
      ...opponent.pokemons[0],
      currentHp: opponent.pokemons[0].hp,
    };

    const fullLog = [];

    fullLog.push(
      `${myPokemon.name} inicia con HP:${myPokemon.hp} ATK:${myPokemon.attack} DEF:${myPokemon.defense}`
    );
    fullLog.push(
      `${enemyPokemon.name} inicia con HP:${enemyPokemon.hp} ATK:${enemyPokemon.attack} DEF:${enemyPokemon.defense}`
    );
    ...
    while (myPokemon.currentHp > 0 && enemyPokemon.currentHp > 0) {
      ...
      fullLog.push(
        `${myPokemon.name} ataco a ${enemyPokemon.name} con ${myDamage} de daño (DEF:${enemyPokemon.defense}) ${enemyPokemon.name} HP:${enemyPokemon.currentHp}`
      );
      ...
      fullLog.push(
        `${enemyPokemon.name} ataco a ${myPokemon.name} con ${enemyDamage} de daño (DEF:${myPokemon.defense}) ${myPokemon.name} HP:${myPokemon.currentHp}`
      );
